...
public class GetMembersTool : IAITool
{
    ...
    public string Description => "Lay danh sach thanh vien";
    public JsonObject ParametersSchema => new JsonObject { ["type"] = "object", ["properties"] = new JsonObject { ["group_id"] = new JsonObject { ["type"] = "string" }, ["role"] = new JsonObject { ["type"] = "string" } }, ["required"] = new JsonArray { "group_id" } };
    ...
    public bool ValidateParameters(JsonObject p) => Guid.TryParse(Js(p["group_id"]), out _);
    ...
    public async Task<AIQueryResult> ExecuteAsync(AIQueryContext context, JsonObject parameters, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (!Guid.TryParse(Js(parameters["group_id"]), out var groupId)) return AIQueryResult.Error("Invalid group_id");
            var roleFilter = Js(parameters["role"]);
            if (!await _participantRepository.IsUserInGroupAsync(groupId, context.UserId)) return AIQueryResult.Error("Ban khong co quyen");
            ...
            sw.Stop();
            return AIQueryResult.Success(new JsonObject { ["members"] = new JsonArray(memberList.ToArray()), ["total"] = memberList.Count, ["group_id"] = groupId.ToString() }, sw.ElapsedMilliseconds);
        }
