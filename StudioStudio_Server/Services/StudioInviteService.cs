...
{
    /// <summary>
    /// Service handling studio invitation token management using Redis cache
    /// Manages temporary invite links with expiration and rate limiting
    /// Token lifetime: 15 minutes
    /// Rate limit: 5 invite links per 15 minutes per user per studio
    /// </summary>
    public class StudioInviteService : IStudioInviteService
    {
        private readonly IConnectionMultiplexer _redis;
        private readonly ILogger<StudioInviteService> _logger;
        ...
        public StudioInviteService(
            IConnectionMultiplexer redis,
            ILogger<StudioInviteService> logger)
        {
            _redis = redis;
            _logger = logger;
        }
        ...
        /// Key: "studio:invite:{token}"
        /// Value: JSON serialized StudioInviteToken
        /// TTL: 15 minutes
        ...
        /// Returns: StudioInviteToken if found and not expired, null otherwise
        ...
        public async Task<StudioInviteToken?> GetInviteTokenDataAsync(string token)
        {
            ...
            StudioInviteToken? inviteData = JsonSerializer.Deserialize<StudioInviteToken>(value.ToString());
        ...
        /// Key: "invite:ratelimit:studio:{studioId}:{userId}"
        /// Limit: 5 invites per 15 minutes per user per studio
